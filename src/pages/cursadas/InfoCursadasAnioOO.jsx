...
const InfoCursadasAnio = () => {
  ...
  const mostrarActividades = (sede, nombre) => {
   ...
   // console.log(comisionesFiltro)
    const actividadesUnicas = [...new Set(comisionesFiltro?.map(item => item.mater + ': ' + item.nmat))]
  ...
  const onHandleMostrar = async () => {
    ...
    if (!procesado) {
      setProcesado(true)
      console.log(actividad, sedeSel, periodo,anio, pgenerico)
      const resultadoI = await traerComparativaInscripcionesActividad(anio, sedeSel, actividad.split(':')[0].trim(), pgenerico)
      setDatos(resultadoI)
      const resultadoC = await filtrarComisionesPorActividad(actividad.split(':')[0].trim(), sedeSel, periodo)
      ...
      <Grid container spacing={2} sx={{ my: 2 }}>
        <Grid item xs={12}>
          <Typography variant="h5" textAlign="center" color="white" backgroundColor="#444444" sx={{ borderRadius: 1 }}>
            Informe de Inscriptos por Actividad - Año {anio}
          </Typography>
        </Grid>
        ...
        <Grid item xs={12} md={1}>
          <Button onClick={onHandleMostrar}>Ver</Button>
        </Grid>
      </Grid>
