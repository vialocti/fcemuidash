...
const TurnosMesasComponentCp = ({ onDataFetch }) => {
  ...
  useEffect(() => {
    const fetchResultados = async () => {
      try {
        ...
        if (onDataFetch) {
          onDataFetch(resultadosMap); // Enviamos los resultados al padre
        }
      } catch (error) {
        console.error('Error al obtener turnos:', error);
      }
 ...
 //buscar mesas
  ...
  return (
    <Container maxWidth="fluid" sx={{ mt: 4 }}>
      <Paper elevation={3} sx={{ p: 2 }}>
        ...
        <Grid container spacing={2}>
          ...
          {/* Botón de búsqueda */}
      <Grid item xs={12} md={3} sx={{ display: "flex", justifyContent: "center", mb: 2 }}>
        <Button
          variant="contained"
          color="primary"
      ...
      {resultados && (
        <Paper elevation={4} sx={{ p: 2, borderRadius: 3 }}>
          <Typography variant='h6'>
            Total de Inscriptos: {(resultados.A + resultados.R + resultados.U)} de estudiantes
          </Typography>
          <Table>
            <TableHead>
              <TableRow>
          ...
          <Typography variant='h6'>
            Porcentaje de Aprobados Turno(referido al total de estudiantes que se presentan): {((resultados.A / (resultados.A + resultados.R )) * 100).toFixed(2)}%
          </Typography>
        </Paper>
      )}
